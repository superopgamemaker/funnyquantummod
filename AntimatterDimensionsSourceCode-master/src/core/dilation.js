...
export function getDilationGainPerSecond() {
  if (Pelle.isDoomed) {
    const tachyonEffect = Currency.tachyonParticles.value.pow(PelleRifts.paradox.milestones[1].effectOrDefault(1)*1.025);
  ...
  let dtRate = new Decimal(Currency.tachyonParticles.value.pow(1.025))
    .timesEffectsOf(
