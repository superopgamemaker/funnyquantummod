...
export const GlyphGenerator = {
  ...
  randomGlyph(level, rngIn, typeIn = null) {
    ...
    if (type !== "effarig" && numEffects > 5) numEffects = 5;
  ...
  randomNumberOfEffects(type, strength, level, rng) {
    ...
    if (type !== "effarig" && Ra.unlocks.glyphEffectCount.canBeApplied) return 5;
    const maxEffects = Ra.unlocks.glyphEffectCount.canBeApplied ? 8 : 5;
    let num = Math.min(
      maxEffects,
