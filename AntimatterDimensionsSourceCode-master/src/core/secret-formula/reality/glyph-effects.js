...
export const glyphEffects = {
  timepow: {
    ...
    effect: (level, strength) => 1.01 + Math.pow(level, 0.3232) * Math.pow(strength, 0.45) / 75,
  ...
  timespeed: {
    ...
    effect: (level, strength) => (GlyphAlteration.isEmpowered("time")
      ? 1 + Math.pow(level, 0.3535)
      : 1 + Math.pow(level, 0.303) * Math.pow(strength, 0.65) / 20),
    formatEffect: x => format(x, 3, 3),
  ...
  timeetermult: {
    ...
    effect: (level, strength) => Math.pow((strength + 3) * (level ** 1.01) , 0.9) *
  ...
  timeEP: {
    ...
    shortDesc: () => (GlyphAlteration.isAdded("time")
      ? "EP ×{value} and ^{value2}"
      : "EP ×{value}"),
    effect: (level, strength) => Math.pow((level ** 1.01) * strength, 3) * 100,
  ...
  dilationDT: {
    ...
    effect: (level, strength) => (GlyphAlteration.isEmpowered("dilation")
      ? DC.D1_005.pow(level ** 1.01).times(15)
      : Decimal.pow((level ** 1.01) * strength, 1.5).times(2)),
  ...
  dilationgalaxyThreshold: {
    ...
    effect: (level, strength) => 1 - Math.pow(level, 0.1717) * Math.pow(strength, 0.35) / 100 -
  ...
  dilationTTgen: {
    ...
    shortDesc: () => (GlyphAlteration.isAdded("dilation")
      ...
      : "{value} TT/hr"),
    effect: (level, strength) => Math.pow((level ** 1.01) * strength, 0.5) / 10000,
  ...
  dilationpow: {
    ...
    effect: (level, strength) => 1.1 + Math.pow(level, 0.707) * Math.pow(strength, 0.7) / 25,
    formatEffect: x => format(x, 2, 2),
  ...
  replicationspeed: {
    ...
    effect: (level, strength) => (GlyphAlteration.isEmpowered("replication")
      ? DC.D1_007.pow(level ** 1.01).times(10)
      : Decimal.times((level ** 1.01), strength).times(3)),
    formatEffect: x => format(x, 2, 1),
    combine: GlyphCombiner.multiplyDecimal,
  ...
  replicationpow: {
    ...
    effect: (level, strength) => 1.1 + Math.pow(level, 0.505) * strength / 25 +
  ...
  replicationdtgain: {
    ...
    effect: (level, strength) => 0.0003 * Math.pow(level, 0.303) * Math.pow(strength, 0.65),
  ...
  replicationglyphlevel: {
    ...
    effect: (level, strength) => Math.pow(Math.pow(level, 0.2525) * Math.pow(strength, 0.4), 0.5) / 50,
    formatEffect: x => format(x, 3, 3),
    combine: effects => {
  ...
  infinitypow: {
    ...
    effect: (level, strength) => 1.007 + Math.pow(level, 0.2121) * Math.pow(strength, 0.4) / 75 +
  ...
  infinityrate: {
    ...
    effect: (level, strength) => Math.pow(level, 0.202) * Math.pow(strength, 0.4) * 0.04,
    formatEffect: x => format(x, 2, 2),
    combine: GlyphCombiner.add,
    enabledInDoomed: true,
  },
  infinityIP: {
    ...
    shortDesc: () => (GlyphAlteration.isAdded("infinity")
      ? "IP ×{value} and ^{value2}"
      : "IP ×{value}"),
    effect: (level, strength) => Math.pow((level ** 1.01) * (strength + 1), 6) * 10000,
    formatEffect: x => format(x, 2, 3),
    combine: GlyphCombiner.multiply,
  ...
  powerpow: {
    ...
    shortDesc: () => (GlyphAlteration.isAdded("power")
      ...
      : "AD power +{value}"),
    effect: (level, strength) => 1.015 + Math.pow(level, 0.202) * Math.pow(strength, 0.4) / 75,
    formatEffect: x => format(x, 3, 3),
    formatSingleEffect: x => format(x - 1, 3, 3),
    combine: GlyphCombiner.addExponents,
    conversion: x => 2 / (x + 1),
  ...
  powermult: {
    ...
    effect: (level, strength) => (GlyphAlteration.isEmpowered("power")
      ? DC.D11111.pow((level * 220) ** 1.01)
      : Decimal.pow((level ** 1.01) * strength * 10, (level ** 1.01) * strength * 10)),
  ...
  powerdimboost: {
    ...
    effect: (level, strength) => Math.pow((level ** 1.01) * strength, 0.5) *
  ...
  powerbuy10: {
    ...
    effect: (level, strength) => 1 + (level ** 1.01) * strength / 12,
    formatEffect: x => format(x, 2, 2),
    combine: GlyphCombiner.addExponents,
    enabledInDoomed: true,
  },
  effarigrm: {
    ...
    effect: (level, strength) => (GlyphAlteration.isEmpowered("effarig")
      ? Math.pow(level, 1.515)
      : Math.pow(level, 0.606) * strength),
    formatEffect: x => format(x, 2, 2),
    combine: GlyphCombiner.multiply,
  ...
  effarigglyph: {
    ...
    effect: (level, strength) => Math.floor(10 * Math.pow((level ** 1.01) * strength, 0.5)),
  ...
  effarigblackhole: {
    ...
    effect: (level, strength) => 1 + Math.pow(level, 0.2525) * Math.pow(strength, 0.4) / 75,
    formatEffect: x => format(x, 3, 3),
    formatSingleEffect: x => format(x - 1, 3, 3),
    combine: GlyphCombiner.addExponents,
  },
  effarigachievement: {
    ...
    effect: (level, strength) => 1 + Math.pow(level, 0.404) * Math.pow(strength, 0.6) / 60 +
  ...
  effarigforgotten: {
    ...
    effect: (level, strength) => 1 + 2 * Math.pow(level, 0.2525) * Math.pow(strength, 0.4),
    formatEffect: x => format(x, 2, 2),
    combine: GlyphCombiner.multiply,
  ...
  effarigdimensions: {
    ...
    effect: (level, strength) => 1 + Math.pow(level, 0.2525) * Math.pow(strength, 0.4) / 500,
    formatEffect: x => format(x, 3, 3),
    formatSingleEffect: x => format(x - 1, 3, 3),
    combine: GlyphCombiner.addExponents,
  },
  effarigantimatter: {
    ...
    effect: (level, strength) => 1 + Math.pow(level, 0.2525) * Math.pow(strength, 0.4) / 5000,
  ...
  timeshardpow: {
    ...
    effect: (level, strength) => 1 + (strength / 3.5) * Math.pow(level, 0.3535) / 400,
    formatEffect: x => format(x, 3, 3),
    formatSingleEffect: x => format(x - 1, 3, 3),
    combine: GlyphCombiner.addExponents,
    enabledInDoomed: true,
  ...
  realityglyphlevel: {
    ...
    effect: level => Math.floor(Math.sqrt((level * 90) ** 1.01)),
    formatEffect: x => formatInt(x),
    combine: GlyphCombiner.add,
  },
  realitygalaxies: {
    ...
    totalDesc: "All Galaxy strength +{value}",
    shortDesc: "Galaxy Strength +{value}",
    effect: level => 1 + Math.pow((level ** 1.01) / 100000, 0.5),
  ...
  realityrow1pow: {
    ...
    effect: level => 1 + (level ** 1.01) / 125000,
    formatEffect: x => format(x, 3, 3),
    combine: GlyphCombiner.addExponents,
  },
  realityDTglyph: {
  ...
  powercost: {
    ...
    isGenerated: true,
    glyphTypes: ["power"],
    singleDesc: "Cost scaling post-e308 reduced by x{value}",
    totalDesc: "Cost scaling post-e308 reduced by x{value}",
    shortDesc: "Cost scaling reduced",
    effect: (level, strength) => pow(1 + ((level ** 0.101) * (strength ** 0.2)), -0.01),
    formatEffect: x => format(x, 2, 2),
    combine: GlyphCombiner.multiply,
    enabledInDoomed: true
  }
